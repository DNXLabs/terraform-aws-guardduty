...
resource "aws_iam_role" "guardduty_notification_iam_role" {
  ...
  name               = "guardduty_notification_iam_role_lambda-${data.aws_region.current.name}"
...
resource "aws_iam_policy" "guardduty_notification_lambda_logging_policy" {
  count       = var.enabled ? 1 : 0
  name        = "guardduty_notification_lambda_logging_policy-${data.aws_region.current.name}"
  path        = "/"
...
resource "aws_cloudwatch_log_group" "guardduty_notification_log_group" {
  count             = var.enabled ? 1 : 0
  name              = "/aws/lambda/${var.lambda_name}-${data.aws_region.current.name}"
